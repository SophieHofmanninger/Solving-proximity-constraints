...
class MatrixJUnit {
	...
	/**
	 * Test method for {@link tool.Matrix#Matrix(int)} and
	 * {@link tool.Matrix#getSize()}.
	 */
	@Test
	void testMatrix() {
		m= new Matrix(0);
	 ...
	 */
	@Test
	void testPutGetAt() throws Exception{
		m=new Matrix(8);
		Tuple<Integer> pos1 = new Tuple<Integer>(2,3);
		Tuple<Integer> pos2 = new Tuple<Integer>(3,2);
		m.putAt(pos1, 0.5f);
		
		// Test Symmetry
		assertEquals(0.5f,m.getAt(pos1));
		assertEquals(0.5f,m.getAt(pos2));
		
		// Test Main diagonal
		for(int i=0;i<8;i++) {
			assertEquals(1f,m.getAt(new Tuple<Integer>(i,i)));
		}
		
		/* Test Errors:
		 * 1. PutErrors
		 * 1.1 Index smaller 0
		 * 1.2 Index greater size
		 * 1.3 Indices equal
		 * 1.4 Value not in [0,1]
		 * 2. Get Errors
		 * 2.1 Index smaller 0
		 * 2.2 Index greater size
		 */
